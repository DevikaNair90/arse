...
arse_ts <- function(data, xcoord, ycoord, ybase = NA, yend = NA, yinvert = FALSE, saveout = FALSE ){
  ...
  ifelse(arse_t < 0, arse_ts <- arse_t * (yend/ybase), arse_ts <- arse_t * (ybase/yend))
  if(saveout == FALSE) return(arse_ts)
  if(saveout == TRUE) as.data.frame(data) %>% mutate(arse_ts = arse_ts)
}
